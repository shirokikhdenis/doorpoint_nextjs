...
function CatalogPageContent({ initial }: CatalogPageClientProps) {
  const [meta, setMeta] = useState<CatalogMeta>(initial.meta);
  const filters = useCatalogFilters(meta, {
    initialCatalogPage: initial.catalogPage,
        ...
        <div className="flex w-full flex-1 flex-col gap-4 lg:flex-row lg:items-start lg:gap-6">
          ...
          >
            <CatalogFilterSidebar
              ...
              onOnSaleChange={filters.setOnSale}
...
export function CatalogPageClient({ initial }: CatalogPageClientProps) {
  return (
    <Suspense
      fallback={
    ...
    >
      <CatalogPageContent initial={initial} />
    </Suspense>
  );
}
