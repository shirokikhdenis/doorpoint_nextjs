...
export const POST = async (request) =>
  withErrorHandling(async () => {
    const limit = checkRateLimit(`cart:${clientIp(request)}`);
    if (!limit.allowed) {
      return json(
    ...
    const result = await leadService.createCartLead(body, { sourcePage });

    if (!result.ok) {
      return json({ message: result.message || "Ошибка отправки" }, result.status || 500);
    }

    const mailResult = await measureLeadService.sendCartLeadMail({
      name: result.lead.customerName,
      phone: result.lead.phone,
      comment: result.lead.clientComment,
      items: result.lead.items.map((item) => ({
    ...
    if (!mailResult.ok) {
      console.error("[cart-lead] email failed:", mailResult.message);
    }
